...
async function run() {
  const { data, error } = await supabase.from('tasks').select('*, employee_positions(employee_name)').limit(1);
  console.log('join error 1:', error);
  const { data: d2, error: e2 } = await supabase.from('tasks').select('*, profiles(id, employee_name)').limit(1);
  console.log('join error 2:', e2);
}
run();
